...
using WebClient.Plugins.Survey;
...
namespace WebClient.Plugins
{
    public class Tickets
    {
        ...
        public string GeneraEncuesta()
        {
            var query = dbc.TICKETs.Where(x => x.SEND_SURVEY == false)
                .Where(x => x.ID_STAT_END == 6)
                .Where(x => x.ID_ACCO == 4)
                .Where(x => x.ID_TYPE_FORM == null)
                .Where(x => x.ID_TICK == 88269).ToList();
                //.Where(x => x.ID_TICK >= 88204).ToList();
        ...
        private int InsertaPreguntas(TICKET tic)
        {
            ...
            try
            {
                var survey = dbe.SURVEY_ACCOUNT_TYPE_TICKET
                    ...
                    .Where(x=>x.ID_TYPE_TICK == tic.ID_TYPE_TICK.Value)
                    .Single(x=>x.DATE_TO == null);
        ...
        public bool EnviarEncuesta(TICKET tic, int ID_SURV_TICK)
        {
            try
            {
                ...
                var html_msg = bodySurvey.htmlSendSurvey(pe, ce, ID_SURV_TICK, cq);
                ...
                //Logo e-data-group
                LinkedResource btnedgw = new LinkedResource(serv + "\\logo-e-data-group-w.png", MediaTypeNames.Image.Jpeg);
